...
const SettingBox = () => {
  ...
  return (
    ...
    >
      <Section $justify="center" $width="100%" $custom="h3{text-align:center; width: 100%}">
      ...
      <Section>
        ...
        <Section>
          ...
          <Section $direction="row" $custom="fill: var(--color-2); gap: 3px;">
            <DarkThemeIcon $width="30px" $height="30px" $theme={$theme} $themeData={$themeData} />
            <SwitchButton
              $isActive={$theme === $themeData[1]}
              onClick={$toggleTheme}
            />
            <LightThemeIcon $width="30px" $height="30px" $theme={$theme} $themeData={$themeData} />
          </Section>
        </Section>

        <Section $custom="fill: var(--color-2)">
          <span>{library.detail}<TipIcon  $width='18px'$tip={library.detailHelpText}/></span>
